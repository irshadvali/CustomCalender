...
class DayItem extends Component {
  ...
  componentWillReceiveProps(newProps) {
    if (newProps.itemvalue.currentMonth == 0) {
      this.setState({
        containerStructure: styles.otherMonthDayContainer
      });
    } else {
  ...
  textStylefunction() {
    if (this.props.itemvalue.day == 0) {
      if (this.props.itemvalue.currentMonth == 0) {
        return (
          <Text style={styles.unseenTextRed}>
            {" "}
            {this.props.itemvalue.monthdate}
          </Text>
        );
      } else {
        if (
      ...
      if (
        ...
        return (
          <Text style={styles.yearText}> {this.props.itemvalue.monthdate}</Text>
        );
      }
    }
  }
